-- Window management. Everything in this file is native `hl.*`, so it works on
-- any Hyprland install without a single extra package.

o.bind("SUPER + W", "Close window", hl.dsp.window.close())

-- Close everything, then land on workspace 1.
o.bind("CTRL + ALT + DELETE", "Close all windows", function()
...
-- Pop the window out of the tiling: float it, size it, pin it above the rest,
-- and keep it there across workspace switches. Same keys pop it back in.
o.bind("SUPER + O", "Pop window out (float & pin)", function()
...
-- Switch the current workspace between the dwindle (BSP) and scrolling
-- (niri-like) layouts. Workspaces start on the layout set in looknfeel.lua,
-- and this only lasts until the compositor restarts.
o.bind("SUPER + L", "Toggle workspace layout", function()
  ...
  hl.exec_cmd(o.notify("Workspace layout set to " .. layout, { urgency = "low" }))
end)
...
-- code:10 through code:19 are the number row, so this holds whatever the
-- keyboard layout prints on those keys.
...
-- code:20 and code:21 are - and = on a US layout: the keys left of Backspace.
...
-- Swap the left and right columns of the current workspace. The script is
-- vendored in bin/ next to this config; it needs jq.
o.bind("SUPER + SHIFT + J", "Invert workspace columns", require("programs").invert_columns)
